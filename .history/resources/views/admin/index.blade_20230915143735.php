...
<script>
    $(document).ready(function() {
        // Add a click event listener to each row with the "data-toggle" attribute
        $('tr[data-toggle="modal"]').click(function() {
            // Get the item ID from the clicked row's data attribute
            var itemId = $(this).data('item-id');

            // Retrieve the item's data (replace with your own data retrieval logic)
            var item = getItemDataById(itemId);

            // Populate the "detail data" modal with the retrieved data
            $('#detailName').text(item.name);
            $('#detailDob').text(item.dob);
            $('#detailArea').text(item.area);
        });

        // Replace this function with your own data retrieval logic
        function getItemDataById(itemId) {
            // Simulated data retrieval based on item ID (replace with your logic)
            var data = {
                name: "John Doe",
                dob: "1990-01-01",
                area: "Sample Area"
